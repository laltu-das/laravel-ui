  ...
  const bindClasses = computed(() => {
    ...
    if (isGradient && isOutline) {
      ...
      if (!simpleGradients.includes(props.gradient.value)) {
        backgroundClass =
            buttonOutlineGradientClasses.default[props.gradient.value]

        if (!props.disabled.value)
          hoverClass = buttonOutlineGradientClasses.hover[props.gradient.value]
      } else {
        console.warn(
            `cannot use outline prop with "${props.gradient.value}" gradient`
        ) // TODO: prettify
      }
    } else if (isGradient) {
      // JUST GRADIENT
      ...
      if (!alternativeColors.includes(props.color.value)) {
        ...
        console.warn(
            `cannot use outline prop with "${props.color.value}" color`
        ) // TODO: prettify
      }
    } else {
      // JUST COLOR
      ...
      if (!props.disabled.value) hoverClass = buttonColorClasses.hover[color]
    }
    ...
    if (props.shadow.value === "") {
      ...
      if (
          props.gradient.value &&
          simpleGradients.includes(props.gradient.value)
      ) {
    ...
    return twMerge(
        ...
        (slots.prefix || slots.suffix || props.loading.value) &&
